...
/* Add by archer for IPv6 */
int (*sc_get_status_p)(void) = NULL;
int (*sc_write_ipv6_p) (struct in6_addr *addr, unsigned int prelen) = NULL; 
int (*sc_get_ipv6_p)(struct in6_addr *addr) = NULL;
int (*sc_write_route_p) (struct in6_addr *addr, unsigned int prelen) = NULL; 
/* Add end */
...
#ifdef IPV6_LOGO_TEST
static void dump_v6addr( struct in6_addr *inaddr )
{
	int i=0;

	for(i=0;i<16;i++)
		printk("%X:", inaddr->s6_addr[i]);
	printk("\n");
...
#if 0 //def IPV6_LOGO_TEST
	printk("%s(%d) target: \n", __func__, __LINE__);
	dump_v6addr(&ifp->addr);
#endif
		       ...
		       struct in6_addr *daddr, struct in6_addr *saddr)
{
	...
	for (dev = dev_base; dev; dev=dev->next) {
		 ...
		 *    interface.)
		 */
#if 0 //kylin: comment for ipv6 (addr.p2, case 30,31)
...
//#ifndef IPV6_LOGO_TEST
/* Add by archer for auto IPv6 */
    if(sc_get_status_p == NULL ? 0:(sc_get_status_p)())
        return;
/* Add end */
//#endif
	/*
...
#if 1
				/* Add by archer for write address to cfg */
				if(ifp != NULL) {
    			    /* Remove old IPv6 address */    			    
    			    if(sc_get_ipv6_p != NULL) {
    			        struct in6_addr oldaddr; 
    			        struct inet6_ifaddr * oldifp;
    			         
    			        (sc_get_ipv6_p)(&oldaddr);

    			        oldifp = ipv6_get_ifaddr(&oldaddr, dev, 1);
#if 0  //kylin: for ipv6 test (nd.p2, case 128)
    			        if(oldifp != NULL)
    			            ipv6_del_addr(oldifp);
#endif 
    			    }			    
    			    /* write to cfg */
    			    if (sc_write_ipv6_p != NULL) 
    			        (*sc_write_ipv6_p)(&ifp->addr, ifp->prefix_len);
    			    
        		    if(sc_write_route_p != NULL)
        		        (sc_write_route_p)(&pinfo->prefix, pinfo->prefix_len);
			    }
			    /* Add end */
#endif
			}
...
static void addrconf_dev_config(struct net_device *dev)
{
	...
	//Shearer LU: Don't assign IPv6 Address in ethx or athxx interfaces.
...
#ifdef IPV6_LOGO_TEST
	if (ifp->idev->if_flags & IF_RA_RCVD || ra_flag_rcvd) {
#else
...
static void addrconf_dad_timer(unsigned long data)
{
	...
	if( !strcmp(ifp->idev->dev->name, "br0")){
		printk("<br0> not br0 found, do not set timer, !\n");
		//return;
	}
	if (ifp->idev->if_flags & IF_RA_RCVD || ra_flag_rcvd) {
		//	return;
	} else
		printk("=== [k] %s(%d) kflag %d\n", __func__, __LINE__, kflag);
...
/* Add by archer for IPv6 address */
EXPORT_SYMBOL (sc_get_status_p);
EXPORT_SYMBOL (sc_write_ipv6_p);
EXPORT_SYMBOL (sc_get_ipv6_p);
EXPORT_SYMBOL (sc_write_route_p);
/* Add end */
