		   ...
		   struct in6_addr *daddr, struct in6_addr *saddr) 
{
	...
	printk("==== *** send ns %d, dev %s\n", __LINE__, dev->name);
	printk("=== %s(%d) daddr: ", __func__, __LINE__);
	dump_v6addr( daddr);
	printk("saddr: ");
	dump_v6addr( saddr);
#endif
...
static void ndisc_recv_ns(struct sk_buff *skb)
{
	...
	dump_v6addr( saddr);
	printk("target: ");
	dump_v6addr( &msg->target);
	printk("Is dad: %d, dad_failed %d\n", dad, dad_failed);
#endif
	if (dad && dad_failed) {
		printk("%s(%d): dad_failed %d, return\n", __func__, __LINE__, dad_failed);
		return;
	}
#endif
...
#ifdef IPV6_LOGO_TEST
	// kylin add for ipv6 nd.p2 v6LC_2_3_6_g.seq . start
...
#endif
	// kylin add for ipv6 nd.p2 v6LC_2_3_6_g.seq . end
