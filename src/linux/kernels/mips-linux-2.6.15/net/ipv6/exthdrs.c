...
looped_back:
	...
	// kylin: fix spec.p2, bug id 40,41
	//printk(" === %s(%d), ipv6 rt hdr type %x\n", __func__, __LINE__, hdr->type);
	if (hdr->type == 0x21 || hdr->type == 0) {
		//printk(" === %s(%d), ipv6 rt hdr type %x, got it!\n", __func__, __LINE__, hdr->type);
			icmpv6_send(skb, ICMPV6_PARAMPROB, 0,
				    0x2a, skb->dev);
			kfree_skb(skb);
			return -1;
	} 
	// kylin: fix spec.p2, bug id 40,41 . END
#endif
