...
static int icmpv6_rcv(struct sk_buff **pskb, unsigned int *nhoffp)
{
	...
	case ICMPV6_ECHO_REQUEST:
		...
		/* kylin: fix icmp.p2 7.html bug */
			LIMIT_NETDEBUG("ICMPv6 request [%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x > %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x] saddr %x\n",
				       NIP6(*saddr), NIP6(*daddr), *saddr);
			if(daddr->in6_u.u6_addr32[0] == 0 &&
				daddr->in6_u.u6_addr32[1] == 0 &&
				daddr->in6_u.u6_addr32[2] == 0 &&
				daddr->in6_u.u6_addr32[3] == 1
					)
			{
		//		LIMIT_NETDEBUG("Got it, discard !\n");
				goto discard_it;
			}
		/* kylin: fix icmp.p2 7.html bug */
#endif
