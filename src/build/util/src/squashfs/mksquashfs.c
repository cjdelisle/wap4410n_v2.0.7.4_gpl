...
struct dir_info *dir_scan1(char *, int (_readdir)(char *, char *, struct dir_info *, struct stat *,int *));
...
/* Add by archer */
static int interpret_table_entry(char *line, char* pathname, char* dirname, struct stat* devtable_inode_info)
{
	char type, name[64]={0};
	char* full;
	...
	if (sscanf (line, "%s %c %lo %lu %lu %lu %lu %lu %lu %lu",
	...
	devtable_inode_info->st_ino++;
	
	return 0;
}

int devtable_readdir( char *pathname, char *dir_name, struct dir_info *dir, struct stat* devtable_inode_info)
{
	char *line;
	int status = FALSE;
	...
	do
	{
		if (getline(&line, &length, devtable) != -1) {
			...
			if (len && *line != '#') {
				commentLine = FALSE;
    			if (interpret_table_entry(line, pathname, dir_name, devtable_inode_info))
    				status = FALSE;
			}
			else
			{
...
int scan1_encomp_readdir(char *pathname, char *dir_name, struct dir_info *dir, struct stat *devtable_inode_info, int *status)
{
	int i, n, pass;
	char *basename;
	static int index = 0;
...
int scan1_single_readdir(char *pathname, char *dir_name, struct dir_info *dir, struct stat *devtable_inode_info, int *status)
{
	struct dirent *d_name;
	int i, pass;
...
int scan1_readdir(char *pathname, char *dir_name, struct dir_info *dir, struct stat *devtable_inode_info, int *status)
{
	struct dirent *d_name;
	...
	else if(strncmp(dir->pathname, dev_path, strlen(dir->pathname)) == 0)
	{
	    if(devtable_readdir(pathname, dir_name, dir, devtable_inode_info))
	    {
	        *status = 1;
	        return 1;
	    }
...
void dir_scan(squashfs_inode *inode, char *pathname, int (_readdir)(char *, char *, struct dir_info *, struct stat *,int *))
{
...
struct dir_info *dir_scan1(char *pathname, int (_readdir)(char *, char *, struct dir_info *, struct stat *, int *))
{
	...
	while(_readdir(filename, dir_name, dir, &buf, &status) != FALSE) {

		if(strcmp(dir_name, ".") == 0 || strcmp(dir_name, "..") == 0)
			continue;

        /* Add by archer */
//        if(strncmp(filename, dev_path, strlen(dev_path)) != 0) {
        if(status == 0)
        /* Add end */
        ...
        if(access(filename, F_OK) == 0 || strncmp(filename, log_path, strlen(log_path)) == 0) {
            if(lstat(filename, &buf) == -1) {
        		char buffer[8192];
        		sprintf(buffer, "Cannot stat dir/file %s, ignoring", filename);
        		perror(buffer);
...
int main(int argc, char *argv[])
{
	...
	for(; i < argc; i++) {
		if(strcmp(argv[i], "-processors") == 0) {
			...
			devtable = fopen(argv[i], "r");
			if (fstat(fileno(devtable), &buf) < 0)
				ERROR("%s: file isn't open\n", argv[i]);
			if (buf.st_size < 10)
...
/* Add by Archer */
    ...
    snprintf(log_path, sizeof(log_path), "%s/dev/log", source_path[0]);    
/* Add end */
