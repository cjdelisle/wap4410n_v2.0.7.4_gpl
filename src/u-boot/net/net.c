...
unsigned char broadcast[8]= {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
...
int do_loopback(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	for(i=0;i<256;i++)
	{
		memset(sendbuf+i*4,0x55,2);
		memset(sendbuf+i*4+2,0xaa,2);
	}	
	memcpy(sendbuf, broadcast,6);  // dest
	memcpy(sendbuf+6, NodeAddress,6);   // src	

	dumpData(sendbuf, 256);
...
NetReceive(volatile uchar * inpkt, int len)
{
	...
	printf("NetReceive : packet received\n");
	dumpData(inpkt, len);
#endif
	...
	if(loop_back)
	{
		if(cnt_lp < 10)
			dumpData(inpkt, len);
		cnt_lp++;
		if(memcmp(inpkt, sendbuf, 256)==0)
		{
			printf("SEND == GET.sent %08x, got %08x\n", cnt_sent, cnt_lp);
...
#endif
#if 0
	if ((packetHandler == AssignHandler)
			|| (packetHandler == DownloadHandler))
#endif
	if ((packetHandler == _CODE_RAM_ADDR) )
	{
		if ((x == 0x0) || (x == UPGRADE_HW_ETHER)	//download pkt
			|| (x == ASSIGN_HW_ETHER))	//assign pkt
		{
#ifdef _SC_ENET_DEBUG_
			printf("packet received type=0x%x\n", x);
			dumpData(inpkt, len);
#endif
			(*packetHandler)(inpkt, 0, 0, len);
			return;
		}
		else
		{
			//printf("%s %d, ignore pkt type=0x%x\n", __FUNCTION__, __LINE__, x);
			return;
		}
	}	
#endif
