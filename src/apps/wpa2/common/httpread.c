 ...
 *      The files are buffered via internal callbacks from eloop, 
 *      then presented to an application callback routine when completely read
 *      into memory.
 *      May also be used if no file is expected but just to get the header,
 *      including HTTP replies (e.g. HTTP/1.1 200 OK etc.).
 *
 *      This does not attempt to be an optimally efficient implementation,
 *      but does attempt to be of reasonably small size and memory
 *      consumption... assuming that only small files are to be read.
 *      A maximum file size is provided by application and enforced.
 *
 *      It is assumed that the application does not expect any of the 
 *      following:
 *      -- transfer encoding other than chunked
 *      -- trailer fields
 *      It is assumed that, even if the other side requested that the 
 *      connection be kept open, that we will close it (thus http
 *      messages sent by application should have the connection closed field);
 *      this is allowed by http/1.1 and simplifies things for us.
 *
 *      Other limitations:
 *      -- HTTP header may not exceed a hard-coded size.
 *
 *      Notes:
 *      This code would be massively simpler without some of the new
 *      features of HTTP/1.1, especially chunked data.
 */
...
static void httpread_timeout_handler(
        void *eloop_data,
        void *user_ctx
        );
        ...
        )
{
        ...
        if (httpread_debug >= 1) 
                wpa_printf(MSG_INFO, "httpread timeout (%p)", h);
        ...
        )
{
        do {
                if (word_eq(hbp, "CONTENT-LENGTH:")) {
                        while (isgraph(*hbp)) hbp++;
                        while (*hbp == ' ' || *hbp == '\t') hbp++;
                        if (!isdigit(*hbp)) goto bad;
...
static int httpread_hdr_analyze(struct httpread *h)
{
        ...
        do {
                ...
                if (h->chunked) h->got_content_length = 0;
                ...
                case HTTPREAD_HDR_TYPE_REPLY :
                         ...
                         * do not...
                         */
                        if (h->reply_code != 200) h->max_bytes = 0;
                break;
 ...
 */
static void httpread_read_handler(
        int sd,
        void *eloop_ctx,
        void *sock_ctx
        )
        ...
        do {
                ...
                if (nread == 0) {
                        ...
                        if (!h->got_hdr) {
                                ...
                                if (httpread_debug >= 1) wpa_printf(
                                        MSG_INFO,
                         ...
                         * end
                         */
                        if (httpread_debug >= 10) wpa_printf(MSG_DEBUG,
                                        "httpread ok eof(%p)", h);
                        h->got_body = 1;
                        goto got_file;
                }
                ...
                if (!h->got_hdr) {
                        ...
                        if (httpread_hdr_analyze(h)) {
                                if (httpread_debug >= 1) wpa_printf(MSG_INFO,
                 ...
                 */
                if (!strncasecmp(h->hdr, "SUBSCRIBE", 9) ||
                    !strncasecmp(h->hdr, "UNSUBSCRIBE", 11) ||
                    !strncasecmp(h->hdr, "HEAD", 4) ||
                    !strncasecmp(h->hdr, "GET", 3)) {
                        if (!h->got_body) {
                                if (httpread_debug >= 10)
...
"httpread end chunks(%p)", h);
                                ...
                                if (h->got_content_length && h->body_nbytes >=
                                                h->content_length) {
                                        h->got_body = 1;
                                        if (httpread_debug >= 10) wpa_printf(
                                                MSG_DEBUG,
                                                "httpread got content(%p)", h);
                                        goto got_file;
                                }
                                if (nread <= 0) break;
                                /* Now transfer. Optimize using memcpy
                                 * where we can.
                                 */
                                ...
                                if (h->got_content_length) {
                                        ncopy = h->content_length -
                                                h->body_nbytes;
                                } else {
                                        ncopy = nread;
                                }
                                /* Note: should never be 0 */
                                if (ncopy > nread) ncopy = nread;
                                memcpy(bbp, rbp, ncopy);
                                bbp += ncopy;
                                h->body_nbytes += ncopy;
                                rbp += ncopy;
                                nread -= ncopy;
...
"httpread got content(%p)", h);
        ...
        if (h->body) h->body[h->body_nbytes] = 0; /* null terminate */
        h->got_file = 1;
...
char *httpread_hdr_line_get(struct httpread *h, const char *tag)
{
        ...
        hdr++;
        for (;;) {
                if (!strncasecmp(hdr, tag, tag_len)) {
                        hdr += tag_len;
                        while (*hdr == ' ' || *hdr == '\t') hdr++;
                        return hdr;
                }
