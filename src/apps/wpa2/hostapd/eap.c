...
static void eap_idle_timeout_handler(void *eloop_data, void *user_ctx)
{
        struct eap_sm *sm = eloop_data;
        sm->retransWhile = 0;   /* force retransmission if in IDLE state */
        return;
}
...
SM_STATE(EAP, TIMEOUT_FAILURE)
{
	SM_ENTRY(EAP, TIMEOUT_FAILURE);
        ...
        if (sm->m && (strcmp(sm->m->name, "WPS") != 0)) {
                /* Bug 52473 - changes to WPS state machine, only do this if using WPS */
	        eapol_set_bool(sm, EAPOL_eapTimeout, TRUE);
                return;
        }

        /* Only continue if WPS is being used because we want special handling for WPS timeout */

	if ((sm->m && !sm->m->isDone(sm, sm->eap_method_priv))) {

                /* Bug 52473
                   When WPS times out waiting for a message, we want to send a NACK to the other side.
                   We expect the other side to send a NACK back, after which we finish with an EAP-FAIL
                   Internally WPS is keeping track of the current state and the previous state.
                   When current state == previous state, WPS knows that it has timed out waiting for a 
                   message. So when EAP calls the buildReq() method, it will build a NACK message*/

            	sm->currentId = eap_sm_nextId(sm, sm->currentId);
            	wpa_printf(MSG_DEBUG, "EAP TIMEOUT_FAILURE: building EAP-Request: Identifier %d",
	        	   sm->currentId);
            	sm->lastId = sm->currentId;
        	free(sm->eapReqData);
	        sm->eapReqData = sm->m->buildReq(sm, sm->eap_method_priv,
	        ...
	        if (!sm->eapReqData) { 
                    /* Bug 52473 - changes to WPS state machine
                       EAP-FAIL must be built by EAP itself. If we reach this point, that means we have
                       already sent a NACK message and have not received a NACK back. This is the second
                       timeout, so we must send an EAP-FAIL. WPS will return a NULL request so that EAP
                       knows it must send out an EAP-FAIL. */
		...
		SM_ENTER(EAP, SEND_REQUEST);
                ...
                /* Bug 52473 - changes to WPS state machine
                   We have sent out first the NACK, timed out on the NACK and then sent out the EAP-FAIL
                   The only thing left to do is to make EAP restart with Request Identity*/
	        eapol_set_bool(sm, EAPOL_eapTimeout, TRUE);
        }
...
/* 
 * NOTE that SM_STEP(EAP) is called from eapol a number of times from eapol
 * until no more changes are indicated; then again at the passing of 
 * each second of time, or after receiving data.
 */
SM_STEP(EAP)
{
...
#if 0   /* original ...*/
/* Comment says that EAPOL s.m. does retransmits but i don't see that.
 * Note that the sm fields involved are not really used and who knows
 * what they were supposed to mean since the author provided no comment.
 */
static int eap_sm_calculateTimeout(struct eap_sm *sm, int retransCount,
				   int eapSRTT, int eapRTTVAR,
				   int methodTimeout)
