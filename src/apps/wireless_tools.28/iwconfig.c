	   ...
	   int		count)
{
  ...
  switch(rc)
    ...
    case -ENOTSUP:
/*
* Comment out by BZ@SC_CPUAP   2008.2.1
*       
...
/*
* BZ@SC_CPUAP   2007.7.25
* Add begin
*/
static int
print_current_channel(int		skfd,
	   char *	ifname,
	   char *	args[],
	   int		count)
{
    ...
    if(iw_get_range_info(skfd, ifname, &range) >= 0)
    {
        ...
        if(iw_get_ext(skfd, ifname, SIOCGIWFREQ, &wrq) >= 0)
	    {
	        ...
	        iw_print_freq(buffer, sizeof(buffer), freq, -1, IW_FREQ_FIXED);
	   ...
	   int		count)
{
  ...
  if(iw_get_range_info(skfd, ifname, &range) < 0)
	...
	{
	  printf("available bit-rates :\n");
	  /* Print them all */
	  for(k = 0; k < range.num_bitrates; k++)
	    {
	      rate = range.bitrate[k]; 	
	      printf("%g\n", rate / MEGA);
	    }
	}
      else
...
{
  	struct iwreq		wrq;
    struct wireless_info	winfo, *info;
    char	    buffer[128];	/* Temporary buffer */
    ...
    if(iw_get_range_info(skfd, ifname, &(info->range)) >= 0)
    {
        if(info->range.we_version_compiled > 9)
        {
            /* Get Transmit Power */
            if(iw_get_ext(skfd, ifname, SIOCGIWTXPOW, &wrq) >= 0)
        	{
...
* Add begin
*/	    
      else if(strcmp(argv[2], "getChannel") == 0)
        print_current_channel(skfd, argv[1], NULL, 0);  
	  else if(strcmp(argv[2], "getCurrentChannel") == 0)
		print_channel_info(skfd, argv[1], NULL, 0);
	  else if(strcmp(argv[2], "getCurrentRate") == 0)
		print_rate_info(skfd, argv[1], NULL, 0);
	  else if(strcmp(argv[2], "getCurrTxPower") == 0)
	    print_txpower_info(skfd, argv[1], NULL, 0);
/*
* Add end
*/		
	  else
