...
mDNSexport void mDNS_HostNameCallback(mDNS *const m, AuthRecord *const rr, mStatus result)
	...
	if (result == mStatus_NoError)
		...
		mDNS_SetFQDN(m);
        { // Added by Jick Nan for apply new hostname device scope. 20091218
          // mdns will be restared when hostname is changed.
            char tmp[128] = {0};

            sprintf(tmp, "set hostname %s; apply", (m->hostlabel.c)+1);
            fprintf(stderr, "hostname conflict, restart Bonjour(%s)...", tmp);
            system(tmp);
        }
