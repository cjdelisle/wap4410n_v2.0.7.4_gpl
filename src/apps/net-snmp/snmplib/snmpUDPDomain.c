		 ...
		 void **opaque, int *olength)
{
	...
	while (rc < 0) {
        ...
        if (rc >= 0) {
			...
			t->sock, rc, string));
			#ifdef T_DEBUG    
			    printf("<%s,%d>: recvfrom fd %d got %d bytes (from %s)\n",__FUNCTION__,__LINE__,t->sock, rc, string);
			#endif    
            free(string);
        } else {
...
netsnmp_udp_transport(struct sockaddr_in *addr, int local)
{
    ...
    /*
    * Set the broadcast option      by Breeze_zhang   2007-6-6
    */
{
    int optval = 1;
    if (setsockopt
        (t->sock, SOL_SOCKET, SO_BROADCAST, (char *)&optval,
         sizeof (optval)) != 0) {
        DEBUGMSGTL(("netsnmp_udp", "couldn't set SO_BROADCAST:%s\n", strerror(errno)));
    }
}
