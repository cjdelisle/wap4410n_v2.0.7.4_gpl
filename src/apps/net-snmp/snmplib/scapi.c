...
/* these are older names for newer structures that exist in openssl .9.7 */dsads
...
#if 1   //Add by terry for DES cryptographic
#include "des.c"     
#else
/*******************************************************************-o-******
 * sc_encrypt
...
//#if defined(USE_INTERNAL_MD5) || defined(USE_OPENSSL)
{
    ...
    if (ISTRANSFORM(privtype, DESPriv)) {
        ...
        DES_cbc_encrypt(ciphertext, plaintext, ctlen, key_sch,
                        (DES_cblock *) my_iv, /*DES_DECRYPT*/1);
        *ptlen = ctlen;
    }
#ifdef HAVE_AES
    else if (ISTRANSFORM(privtype, AES128Priv) ||
