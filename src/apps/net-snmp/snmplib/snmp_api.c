 ...
 */
    ...
    char remoteManager[17];
    /* <== Add End */ 
	/* MD@CPU_AP add for linksy at 20080115 */
	char remoteManager_end[17];
	/* add end */ 
int
...
/* MD@CPU_AP add for linksys  at 20080115 */  

			char *p = strrchr(tmp,'.');
			char come_ip_last_part[5];
			char manager_start_ip_last_part[5];
			char manager_end_ip_last_part[5];
			sprintf(come_ip_last_part, "%s", p+1);
			p = strrchr(remoteManager,'.');	
			sprintf(manager_start_ip_last_part, "%s", p+1);
			p = strrchr(remoteManager_end,'.');	
			sprintf(manager_end_ip_last_part, "%s", p+1);
/* add end */
            if((strcmp(remoteManager,"255.255.255.255") != 0) &&
/* MD@CPU_AP change for linksys at 20080115 */
               //(strcmp(remoteManager, tmp) != 0))
				((atoi(manager_start_ip_last_part) > atoi(come_ip_last_part)) ||
				(atoi(manager_end_ip_last_part) < atoi(come_ip_last_part))))
/* change end */
            {
                sp->s_snmp_errno = SNMPERR_BAD_RECVFROM;
                sp->s_errno = errno;
