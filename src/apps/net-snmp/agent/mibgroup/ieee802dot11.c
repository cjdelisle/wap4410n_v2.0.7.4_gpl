...
#define MAX_LEN 100
//static unsigned char tmpstr[MAX_LEN];
//int numOfInterface = 1;/*Number of wireless Interface.*/
                                ...
                                WriteMethod **write_method )
{
  int found = FALSE;
  oid rName [ MAX_OID_LEN ];                            // OID to be returned
/*	long rNameLen = 0,index = 0, index2 = 0 ;
    
 	if( !process_if2indices(vp, rName, &rNameLen, &index, &index2, 4, 
							name, (long *)length, &exact) )
  		return NULL;  
*/
  loadTables();
  memcpy (( char * ) rName, ( char * ) vp->name, ( int ) vp->namelen * sizeof ( oid ));
  for ( np = LIST_FIRST ( &dfList ); np != NULL; np = LIST_NEXT ( np, nodes )) {
    ...
    mylog("df->ifIndex : %d",df->ifIndex);
    rName[vp->namelen + 1] = df->WEPDefaultKeyIndex;
    mylog("df->WEPDefaultKeyIndex : %d",df->WEPDefaultKeyIndex);
    if ((  exact && ( snmp_oid_compare ( rName, vp->namelen + 2, name, *length ) == 0 )) ||
        ( !exact && ( snmp_oid_compare ( rName, vp->namelen + 2, name, *length ) >  0 ))) {
      switch ( vp->magic ) {
  ...
  switch ( action ) {
    ...
    case ACTION:
    	if ( var_val_len <= 32) {
			...
			apCfgSsidSet(unit, bss, (unsigned char *)var_val);
		}	
		break;      	
    case UNDO:
      break;
  ...
  switch ( action ) {
    ...
    case RESERVE1:
      ...
      if ( *(long *)var_val < 1 || *(long *)var_val > 3 ){
      	fprintf ( stderr,"write to dot11DesiredBSSType: value out of range\n" );
  ...
  switch ( action ) {
    ...
    case RESERVE1:
      ...
      if ( *(long *)var_val < 20 || *(long *)var_val> 1000 ){
      	fprintf ( stderr,"write to dot11BeaconPeriod: value out of range\n" );
        return SNMP_ERR_WRONGVALUE;
      }
      break;
            ...
            size_t   name_len)
{
  ...
  switch ( action ) {
    ...
    case RESERVE1:
      ...
      if ( *(long *)var_val < 0 || *(long *)var_val > 99 ){
      	fprintf ( stderr,"write to dot11DTIMPeriod: value out of range\n" );
        return SNMP_ERR_WRONGVALUE;
      }         
      break;
    ...
    case ACTION:
    	if(*(long *)var_val >= 0 && *(long *)var_val <= 99)
		{   		
             scApCfgIdleTimeoutIntervalSet(unit,*(long *)var_val);
    	}
    case UNDO:
      break;
            ...
            size_t   name_len)
{
  ...
  switch ( action ) {
    ...
    case FREE:
    case ACTION:/*
    	mylog("wepdefaultkey set : %d",*(long *)var_val);
   		 if (((*(long *)var_val >= CFG_MIN_SHARED_KEY) && (*(long *)var_val <= CFG_MAX_SHARED_KEY)) || (*(long *)var_val == INVALID_SHARED_KEY))
   		{   
				mylog("int index is %d",*(long *)var_val);
                apCfgDefKeySet(unit, bss,*(long *)var_val);
        }
		break;	*/    	
    case UNDO:
      break;
  ...
  switch ( action ) {
    ...
    case RESERVE1:
      ...
      if ( *(long *)var_val < 1 || *(long *)var_val > 2 ){
      	fprintf ( stderr,"write to dot11PrivacyInvoked: value out of range\n" );
        return SNMP_ERR_WRONGVALUE;
      }         
      break;
    ...
    case ACTION:
    	if ( *(long *)var_val >= 1 && *(long *)var_val <= 2 ) {
    		tmp = (*(long *)var_val == 1) ? 1:0 ;
            apCfgAuthTypeSet( unit, bss, tmp);		
      } 
    case UNDO:
      break;
  ...
  switch ( action ) {
    ...
    case RESERVE1:
      ...
      if ( *(long *)var_val < 0 || *(long *)var_val > 3 ){
      	fprintf ( stderr,"write to dot11WEPDefaultKeyID: value out of range\n" );
        return SNMP_ERR_WRONGVALUE;
      }           
      break;
    ...
    case ACTION:
    	if ( *(long *)var_val >= 0 && *(long *)var_val <= 3 )
    	{
    		apCfgDot1xKeyModeSet(unit,bss,*(long *)var_val);
        }
        break;
    case UNDO:
      break;
  ...
  switch ( action ) {
    ...
    case RESERVE1:
      ...
      if ( *(long *)var_val < 256 || *(long *)var_val > 2346 ){
      	fprintf ( stderr,"write to dot11RTSThreshold: value out of range\n" );
        return SNMP_ERR_WRONGVALUE;
      }           
      break;
    ...
    case ACTION:
    	if ( *(long *)var_val >= 256 && *(long *)var_val <= 2346 )
    	{
              apCfgRtsThresholdSet(unit, *(long *)var_val);
    	}	    	
    case UNDO:
      break;
            ...
            size_t   name_len)
{
  ...
  switch ( action ) {
    ...
    case RESERVE1:
      ...
      if ( *(long *)var_val < 256 || *(long *)var_val > 2346 ){
      	fprintf ( stderr,"write to dot11FragmentationThreshold: value out of range\n" );
        return SNMP_ERR_WRONGVALUE;
      }       
      break;
    ...
    case ACTION:       
    	...
    	{
             var_val[var_val_len] = 0;
             apCfgFragThresholdSet(unit, *(long *)var_val);
    	}    	
            ...
            size_t   name_len)
{
  ...
  switch ( action ) {
    ...
    case RESERVE1:
      ...
      if(*(long *)var_val<0 || *(long *)var_val>11){
      	 fprintf ( stderr, "write to dot11CurrentChannel: value out of range\n" );
      	 return SNMP_ERR_WRONGVALUE;
      	}
      break;
...
{
  int i, j = 0;
  //char tmp_name[10],tmp_unit[2],tmp_bss[2];
  int unit,bss;
  ...
  /*
  * MD@SC_CPUAP  at 2007-8-10
  * Changed For 802.11 mib
  */
  if ( apCfgSsidGet(unit,bss) && strlen ( apCfgSsidGet(unit,bss) )) {
    nSc.haveDesiredSSID = TRUE;
    strcpy ( nSc.desiredSSID, apCfgSsidGet(unit,bss)); 
  }
  ...
  if(apCfgBeaconIntervalGet(unit))
       nSc.beaconPeriod = apCfgBeaconIntervalGet(unit); 
       
  nSc.haveDTIMPeriod = TRUE;
  if(scApCfgIdleTimeoutIntervalGet(unit))
		nSc.DTIMPeriod = scApCfgIdleTimeoutIntervalGet(unit);
  ...
  nPr.haveWEPKeyMappingLength = TRUE;
  nPr.WEPKeyMappingLength = apCfgKeyBitLenGet(unit, bss);
  nPr.haveExcludeUnencrypted = TRUE;
  nPr.excludeUnencrypted = 2;               // 2 => FALSE
  //nPr.haveWEPICVErrorCount = TRUE;
  //nPr.WEPICVErrorCount = 0;
  //nPr.haveWEPExcludedCount = TRUE;
  //nPr.WEPExcludedCount = 0;
  if ( wi->has_range ) {
    if (apCfgAuthTypeGet(unit,bss)) 
    {       
       nPr.privacyInvoked = 1;
       if (wi->key_flags & IW_ENCODE_RESTRICTED) 
          nPr.excludeUnencrypted = 1;
       nPr.WEPDefaultKeyID = apCfgDot1xKeyModeGet(unit,bss);//wepCurrentKey;
    }
  }
  ...
  if ( wi->has_range ) {
    ...
    if(apCfgRtsThresholdGet(unit)>=256 && apCfgRtsThresholdGet(unit)<=2346)
    	nOp.RTSThreshold = apCfgRtsThresholdGet(unit);
  }
    nOp.haveShortRetryLimit = TRUE;
    nOp.shortRetryLimit = 7;
    nOp.haveLongRetryLimit = TRUE;
    nOp.longRetryLimit = 4;    
  
  if (apCfgFragThresholdGet(unit)>=256 && apCfgFragThresholdGet(unit)<=2346) {
    nOp.haveFragmentationThreshold = TRUE;
    //nOp.fragmentationThreshold = wi->frag.value;
    nOp.fragmentationThreshold = apCfgFragThresholdGet(unit);
  }
  /*   
   * Changed end
   */
  ...
  if ( wi->has_range ) { // && wi->freq <= ( double ) 2483000000 ) {  // DSSS frequencies only
    for ( i = 0; i < wi->range.num_frequency; i++ ) {
      if (apCfgRadioChannelGet(unit)>=0 && apCfgRadioChannelGet(unit)<=11) {//(( double ) ( wi->range.freq[i].e * 10 ) * ( double ) wi->range.freq[i].m ) == wi->freq 
        nPd.haveCurrentChannel = TRUE;
        nPd.currentChannel = apCfgRadioChannelGet(unit);//wi->range.freq[i].i; 
      }
    }
  }
