...
init_system_mib(void)
{
    ...
    getPid(pid);
    getVersion(fwver);
    sprintf(version_descr, "Linux, Cisco Small Business %s, Version %s", pid, fwver);
    
    strncpy(sysContact, apCfgSnmpContactGet(), sizeof(sysContact));
    sysContact[strlen(sysContact)] = 0;
    
    strncpy(sysName, apCfgSysNameGet(), sizeof(sysName));
    sysName[strlen(sysName)] = 0;
    
    strncpy(sysLocation, apCfgSnmpLocationGet(), sizeof(sysLocation));
    sysLocation[strlen(sysLocation)] = 0;
#endif
...
{
    static u_long   ulret;
/* MD@CPU_AP changed for fix bug  by 20080115 */
    FILE    *fp;
    char    uptime[20], v[50];
	struct timeval 			tv;
/* change end */
...
#ifdef WAP4410N
        if(oid_index == 4)
        	apCfgSnmpContactSet(buf);
        else if(oid_index == 5)
        	apCfgDescSet(buf);
        else if(oid_index == 6)
	        apCfgSnmpLocationSet(buf);
#endif
        break;
